...
const createTimeOverlayElement = time => {
    ...
    timeLink.addEventListener('click', event => {
        ...
        window.history.pushState(null, '', timeLink.href);
    ...
    timeLink.addEventListener('mouseenter', () => {
        timeLink.style['background-color'] = 'var(--yt-sys-color-baseline--button-chip-background-hover)';
    });
...
const copyImageEventHandler = async event => {
    ...
    try {
        ...
        if (success) {
            ...
            showStatusMessage(imageContainer, 'Failed to copy...', false);
        }
    } catch (error) {
